...
filter_bank_vae = torch.load(FILE_fb, map_location=device)
#filter_bank_vae = torch.normal(size=(1,49,8,3,3), mean=0.0, std=1/100, requires_grad=True, device=device)
...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        # get the structure tensor hashing filters
        sthash_filters = hash_and_get_filter(LR_images, filter_bank_sthash)
        ...
        # forward
        pred_mean, pred_variance, outputs, ker_pred, filter_out = model(LR_images_unfolded, filter_bank_vae, sthash_filters)
